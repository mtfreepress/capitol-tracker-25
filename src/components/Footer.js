...
const Footer = () => {
  const menusRendered = footerMenus.map((menu, i) => {
    const itemsRendered = menu.items.map((item, j) => (
      <li key={String(j)}>
        <a href={item.url}>{item.label}</a>
      </li>
    ));
    return (
      <section css={menuStyle} key={String(i)}>
        <h2>{menu.label}</h2>
  ...
  return (
    <footer css={footerStyle}>
      <div css={footerWrapperStyle}>
        <div css={footerColumnsStyle}>{menusRendered}</div>

        <div css={footerInfoStyle}>
          <span>© {new Date().getFullYear()} Montana Free Press. </span>
          <a href="https://montanafreepress.org/about-mtfp/privacy-policy/">
            Privacy Policy
          </a>
